# Artifact Collection — Back to Category Picker
# From item list (page 3+) → category picker (page 2)
# Run as the player, at player, facing anchor

# Reset page
scoreboard players set @s adv.gui_page 2

# Kill item list + nav arrows
kill @e[type=text_display,tag=Adv.ArtItems,distance=..7]
kill @e[type=interaction,tag=Adv.ArtItems,distance=..7]
...
# Update sub-nav text back to [← Tiers]
