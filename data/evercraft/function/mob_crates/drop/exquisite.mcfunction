# Mob Crates - Drop Exquisite (rarity mob kill)

advancement revoke @s only evercraft:mob_crates/killed_exquisite
...
execute if predicate {"condition":"minecraft:random_chance","chance":0.25} run function evercraft:mob_crates/spawn/exquisite
...
execute if score #inv_full ec.var matches 0 run loot give @s loot evercraft:mob_crates/bonus/exquisite
execute if score #inv_full ec.var matches 1 at @s run loot spawn ~ ~ ~ loot evercraft:mob_crates/bonus/exquisite
