# Mob Crates - Drop Common (rarity mob kill)
# 25% chance to spawn crate directly + bonus artifact/ring drops

advancement revoke @s only evercraft:mob_crates/killed_common
...
execute if predicate {"condition":"minecraft:random_chance","chance":0.25} run function evercraft:mob_crates/spawn/common
...
execute if score #inv_full ec.var matches 0 run loot give @s loot evercraft:mob_crates/bonus/common
execute if score #inv_full ec.var matches 1 at @s run loot spawn ~ ~ ~ loot evercraft:mob_crates/bonus/common
