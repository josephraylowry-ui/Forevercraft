# Party GUI — Leave Party Button Click
...
# Execute leave as the player (no position needed — leave doesn't use @s pos)
execute as @a[tag=Adv.InMenu,distance=..10] if score @s adv.gui_session = #gui_click_sess ec.temp run function evercraft:party/menu/leave
