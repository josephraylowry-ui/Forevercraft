# Iceologer Pet Passive Abilities
# - Frost Magic: Attack damage scaling (cold biomes bonus armor)
# - Ice Touch: Melee hits apply Slowness (tag trigger)

data modify storage evercraft:companions math.string set string entity @e[type=item_display, tag=Pet, predicate=evercraft:companions/check_id, limit=1] item.components."minecraft:profile".properties[{name:"level"}].value
scoreboard players set #value Pets.Calc 0
function evercraft:companions/handler/math/string_to_int

execute if predicate evercraft:companions/cold_biome run attribute @s armor base set 6.0
