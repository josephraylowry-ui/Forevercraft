# Convert mob to Greater Furia
# Stats: +100% health, +5 armor, +30% speed, +100% attack damage, +0.3 KB resist
# Scale: random 50/50 bigger (1.2-1.5x) or smaller (0.5-0.8x)

# Apply attribute modifiers
attribute @s max_health modifier add evercraft:furia/furia 1.0 add_multiplied_total
attribute @s armor modifier add evercraft:furia/furia 5 add_value
attribute @s movement_speed modifier add evercraft:furia/furia 0.3 add_multiplied_total
attribute @s attack_damage modifier add evercraft:furia/furia 1.0 add_multiplied_total
attribute @s knockback_resistance modifier add evercraft:furia/furia 0.3 add_value
...
# Apply name with Greater dark red hex color
data modify storage evercraft:furia/temp color set value "AA0000"
...
tag @s add ec.furia
tag @s add ec.furia.greater
tag @s add ec.furia.processed
tag @s add mob_manager.settings.applied
team join ec.furia.greater @s

# Subtle spawn effect
particle minecraft:small_flame ~ ~1 ~ 0.3 0.5 0.3 0.01 5
playsound minecraft:entity.zombie.ambient hostile @a[distance=..16] ~ ~ ~ 0.4 0.8
