...
execute if data storage eden:settings mob_manager.villager_settings{villagename:"enabled"} as @e[type=villager,tag=!mob_manager.settings.exclude,tag=!mob_manager.settings.villagename.applied] at @s if data entity @s Brain.memories.minecraft:meeting_point.value run function evercraft:mobs/village/name/init

#summon texyt display at meeting point if possible
execute as @e[type=armor_stand,tag=mob_manager.village.name] at @s run function evercraft:mobs/village/name/set_display with entity @s
...
execute as @e[type=text_display,tag=mob_manager.village.name] at @s unless entity @e[type=armor_stand,tag=mob_manager.village.name,distance=..10] run kill @s
...
execute unless data storage eden:settings mob_manager.villager_settings{villagecenter_healing:"disabled"} run function evercraft:mobs/village/healing with storage eden:settings mob_manager.villager_settings

#bell particles
execute as @e[type=armor_stand,tag=mob_manager.village.name] at @s run particle minecraft:wax_on ~ ~.5 ~ .4 .3 .4 .5 1

#rename triggers
execute if data storage eden:settings mob_manager.villager_settings{villagename_rename:"enabled"} as @a[scores={mob_manager.used.bell=1..}] at @s if items entity @s weapon.mainhand minecraft:name_tag run function evercraft:mobs/village/rename/check_hand
...
execute unless data storage eden:settings mob_manager.villager_settings{villagename_msg:"disabled"} as @a[tag=not_at_village] at @s if entity @e[type=armor_stand,tag=mob_manager.village.name,distance=..96] run function evercraft:mobs/village/message/entering with entity @n[type=armor_stand,tag=mob_manager.village.name]
execute as @a[tag=at_village] at @s unless entity @e[type=armor_stand,tag=mob_manager.village.name,distance=..96] run function evercraft:mobs/village/message/exiting with storage eden:temp village