# Apply awakening to offhand weapon via item modifier
# Used when player holds stone in mainhand, artifact in offhand (for rods/bows/shields/tridents)
item modify entity @s weapon.offhand evercraft:weapon_mastery/awaken

# Append mastery lore line to item (so write_back can update lore[-1])
summon hopper_minecart ~ 320 ~ {Tags:["wm_temp"],Items:[]}
item replace entity @e[type=hopper_minecart,tag=wm_temp,limit=1] container.0 from entity @s weapon.offhand
...
item replace entity @s weapon.offhand from entity @e[type=hopper_minecart,tag=wm_temp,limit=1] container.0
