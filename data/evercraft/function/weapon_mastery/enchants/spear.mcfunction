# Spear (Beastlord class) enchantments — applied to hopper minecart item
...
# Looting is primary — Beastlord pets benefit from mainhand looting for drop bonuses
...
# Ench 5+: Sweeping Edge
execute if score #wm_ench_level ec.var matches 5.. store result entity @e[type=hopper_minecart,tag=wm_temp,limit=1] Items[0].components."minecraft:enchantments"."minecraft:sweeping_edge" int 1 run scoreboard players get #wm_ench_level ec.var
