...
execute unless score #ww_xp adv.temp matches 125.. run return run tellraw @p[tag=WW.InMenu,distance=..5] [{text:"✦ ",color:"gold"},{text:"Not enough XP! Need 125 levels.",color:"red"}]
execute as @p[tag=WW.InMenu,distance=..5] store result score #inv_full ec.var run function evercraft:util/check_inv_full
experience add @p[tag=WW.InMenu,distance=..5] -125 levels
execute if score #inv_full ec.var matches 0 run loot give @p[tag=WW.InMenu,distance=..5] loot evercraft:professions/wise_wanderer/crates/exquisite_crate
...
tellraw @p[tag=WW.InMenu,distance=..5] [{text:"✦ ",color:"gold"},{text:"Purchased ",color:"gray"},{text:"Exquisite Crate",color:"light_purple"},{text:" for 125 levels!",color:"gray"}]
