# Roll tier for unknown/custom structures (struct_id 20)
# Gateless RNG system - every chest can roll any tier, weighted by Dream Rate + location
# Run as player, cf.tier contains base tier from dimension/Y-level (1-4)
# NOTE: Tellraw/sound announcements moved to announce_tier.mcfunction (only called when loot spawns)

# Store base tier for reference and tellraw
scoreboard players operation @s cf.base_tier = @s cf.tier

# Get player's dream rate - multiply by 10 for decimal precision
execute store result score @s cf.luck run attribute @s luck get 10

# Cap dream rate at 500 (50 Dreams max)
...
# Linear interpolation: weight = base_weight + (dream_rate/500) * (max_weight - base_weight)
# ============================================================================
...
# #w1 = common ceiling, #w2 = uncommon ceiling, etc.
# ============================================================================
...
# If roll > threshold, upgrade tier
