# Give Miner's Anecdote to player and mark as received
# Run as player, at barrel position

# Insert into the barrel
loot insert ~ ~ ~ loot evercraft:books/miners_anecdote

# Mark player as having received this anecdote
scoreboard players set @s ec.anec_miner 1

# Notify player
tellraw @s [{text:"[Anecdote] ",color:"dark_purple"},{text:"You found a dusty notebook... ",color:"gray",italic:true},{text:"Miner's Anecdote",color:"gray",bold:true}]
